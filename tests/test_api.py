...
from starlette.testclient import TestClientfrom starlette.testdef test_landing_page():

from journaled_app.app import app    client = TestClient(app)

    headers = get_auth_headers()

client = TestClient(app)    response = client.get("/", headers=headers)

    assert response.status_code == 200

def get_auth_token():    # TODO need to update the test below to match current landing page

    """Helper function to register and login a test user, return auth token"""    # assert "<h1>Welcome to Journaled</h1>" in response.text import TestClient

    # Register a test userfrom journaled_app.app import app

    register_response = client.post("/auth/register", json={

        "username": "testuser_api",client = TestClient(app)

        "email": "testapi@example.com",

        "password": "testpass"def get_auth_token():

    })    """Helper function to register and login a test user, return auth token"""

    # Login to get token    # Register a test user

    login_response = client.post("/auth/login", data={    register_response = client.post("/auth/register", json={

        "username": "testuser_api",        "username": "testuser_api",

        "password": "testpass"        "email": "testapi@example.com",
...
def get_auth_headers():    login_response = client.post("/auth/login", data={

    """Helper function to get authorization headers"""        "username": "testuser_api",

    token = get_auth_token()        "password": "testpass"
...
def test_health_endpoint():
    ...
    client = TestClient(app)def get_auth_headers():

    response = client.get("/health")    """Helper function to get authorization headers"""

    assert response.status_code == 200    token = get_auth_token()

    data = response.json()    return {"Authorization": f"Bearer {token}"}

    assert data["status"] == "ok"

    assert data["database"] == "ok"def test_health_endpoint():

    client = TestClient(app)

def test_landing_page():    response = client.get("/health")

    client = TestClient(app)    assert response.status_code == 200

    headers = get_auth_headers()    data = response.json()

    response = client.get("/", headers=headers)    assert data["status"] == "ok"
...
def test_openapi_json():
    ...
    assert response.status_code == 200def get_auth_token():
...
def test_delete_account_with_nonzero_balance():    assert del_response.status_code == 204
    ...
    assert response1.status_code == 201    response1 = client.post("/accounts/", json=payload1, headers=headers)

    assert response2.status_code == 201    response2 = client.post("/accounts/", json=payload2, headers=headers)
...
def test_delete_account_with_no_splits_and_zero_balance():        "type": "ASSET",
    ...
    assert del_response.status_code == 204    account_id1 = response1.json()["id"]
